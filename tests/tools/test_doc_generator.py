...
class TestDocGeneratorInit:
    ...
    def test_creates_output_dir(self, tmp_dir: Path) -> None:
        ...
        out_dir = tmp_dir / "docs" / "output"
        assert not out_dir.exists()
        DocGenerator(out_dir)
        assert out_dir.exists()

    def test_existing_dir_ok(self, tmp_dir: Path) -> None:
        """DocGenerator works with existing directory."""
        DocGenerator(tmp_dir)
...
class TestDocGeneratorCreateMarkdown:
    ...
    def test_creates_markdown_file(self, tmp_dir: Path) -> None:
        """create() with MARKDOWN writes content to .md file."""
        gen = DocGenerator(tmp_dir)
        result = gen.create(
            content="# Hello\n\nWorld",
            filename="test",
            fmt=DocFormat.MARKDOWN,
        )
        assert result == tmp_dir / "test.md"
        assert result.exists()
        assert result.read_text(encoding="utf-8") == "# Hello\n\nWorld"

    def test_markdown_ignores_title(self, tmp_dir: Path) -> None:
...
class TestDocGeneratorCreatePdf:
    ...
    def test_creates_md_fallback(self, tmp_dir: Path) -> None:
        ...
        assert result == tmp_dir / "report.md"
        assert result.exists()
        content = result.read_text(encoding="utf-8")
        assert "PDF content here" in content

    def test_pdf_prepends_title(self, tmp_dir: Path) -> None:
    ...
    def test_pdf_no_title(self, tmp_dir: Path) -> None:
    ...
    def test_create_dispatches_to_pdf(self, tmp_dir: Path) -> None:
...
class TestDocGeneratorCreateDocx:
    ...
    def test_create_docx_with_mock(self, tmp_dir: Path) -> None:
        ...
        try:
            ...
            assert result == tmp_dir / "test.docx"
            assert result.exists()
        except ImportError:
...
class TestDocGeneratorEdgeCases:
    ...
    def test_empty_content(self, tmp_dir: Path) -> None:
    ...
    def test_string_output_dir(self, tmp_dir: Path) -> None:
        """DocGenerator accepts string path as output_dir."""
        gen = DocGenerator(str(tmp_dir / "subdir"))
